            ...
            with c2:
                ...
                if st.button(f"📈 {target_date.month}월 분석하기"):
                    with st.spinner(f"{target_date.month}월 분석 중..."):
                        res = model.generate_content(f"2026년 {target_date.month}월 이미지스톡 시장에서 매해 검색량이 증가하는 일러스트 스테디셀러 테마 3개를 추천 이유, 템플릿 예시와 함께 상세히 추천해줘. 또 그 해에 포함 된 한국과 전세계 공통 기념일이나 행사에 대해서도 알려줘.")
                        st.markdown(res.text)
